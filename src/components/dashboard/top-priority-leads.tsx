      ...
      <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {leads.map((lead) => (
          <li key={lead.id}>
            <Link
              href={lead.href}
              className="flex h-full flex-col rounded-lg border border-border bg-background/60 px-3 py-2.5 transition-colors hover:bg-accent/25 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            >
