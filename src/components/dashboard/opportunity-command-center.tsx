...
export function OpportunityCommandCenter({
  ...
  return (
    <section className="rounded-2xl border border-border/80 bg-card/40 p-4 sm:p-5" aria-label="Opportunity command center">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <Link href="/leads?sort=recommended" className="font-medium text-primary underline-offset-4 hover:underline">
            Recommended leads
          </Link>
          <Link href={worthReviewHref} className="font-medium text-primary underline-offset-4 hover:underline">
            Scraped queue
          </Link>
        </div>
      </div>
        ...
        <ul className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
          {leads.map((lead) => (
            <li key={lead.id}>
              <Link
                href={lead.href}
                className="flex h-full flex-col rounded-xl border border-border/70 bg-background/50 px-3 py-2.5 transition-colors hover:border-primary/35 hover:bg-muted/20"
              >
                <span
                  className={cn(
