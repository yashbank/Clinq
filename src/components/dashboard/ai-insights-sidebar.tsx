      ...
      {isEmpty ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3 sm:p-4">
          ...
          {recommendations.length > 0 ? (
            <Collapsible className="group/rec border-t border-border/60 pt-3">
              <CollapsibleTrigger className="flex w-full items-center justify-between gap-2 rounded-lg px-1 py-1.5 text-left transition-colors hover:bg-muted/20">
                ...
                {visibleRecommendations.length === 0 ? (
                  ...
                  <ul className="mt-2 space-y-2">
                    {visibleRecommendations.map((r) => (
                      <li key={r.id} className="relative rounded-lg border border-border/40 bg-background/20 pr-8">
                        <Link href={r.href} className="block px-2.5 py-2 transition-colors hover:bg-muted/25">
                        ...
                        <button
                          ...
                          className="absolute right-1 top-1 flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground"
                        >
          ...
          <div className="mt-auto space-y-0.5 border-t border-border/60 pt-2.5">
            <p className="mb-1 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Shortcuts</p>
            <Link
              href="/proposals"
              className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-foreground transition-colors hover:bg-muted/30"
            >
...
export function AIInsightsSidebar({
  ...
  return (
    <>
      ...
      >
        <div className="flex h-full flex-col border-l border-border/50 bg-sidebar/35">
          <div className="flex shrink-0 flex-col items-center border-b border-border/50 py-2.5">
            ...
            <button
              ...
              className={cn(
                "mt-2 flex h-10 w-10 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground",
                pinned && "text-primary",
              )}
              aria-pressed={pinned}
            >
        ...
        <div
          className={cn(
            ...
            "group-hover/insights:pointer-events-auto group-hover/insights:w-56 group-hover/insights:border-border/50 group-hover/insights:opacity-100",
            "group-data-[expanded=true]/insights:pointer-events-auto group-data-[expanded=true]/insights:w-56 group-data-[expanded=true]/insights:border-border/50 group-data-[expanded=true]/insights:opacity-100",
          )}
        >
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetTrigger asChild>
          <button
            type="button"
            className="fixed bottom-[5.5rem] right-4 z-40 flex h-11 w-11 items-center justify-center rounded-full border border-border/60 bg-sidebar/90 text-primary shadow-md backdrop-blur-sm transition-transform hover:scale-[1.02] active:scale-[0.98] md:hidden"
            aria-label="Open insights"
          >
            <PanelRight className="h-5 w-5" />
          </button>
        </SheetTrigger>
        <SheetContent side="right" className="w-[min(100%,20rem)] border-border p-0 sm:max-w-sm">
